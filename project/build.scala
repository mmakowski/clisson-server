...
object ClissonServerBuild extends Build {
  ...
  val filesToInclude = SettingKey[Seq[(String, String)]]("files-to-include", "Files to include in the distributable zip, in addition to one-jar jar")
  ...
  val zipSettings = Seq(
    zip <<= (target, name, version, oneJar, filesToInclude in zip, streams) map { (buildTarget, projectName, projectVersion, oneJarFile, extraFiles, s) =>
      ...
      val entries = extraFiles.map { case (f, e) => (file(f), dirName + "/" + e) } :+ (oneJarFile, dirName + "/clisson-server.jar")
